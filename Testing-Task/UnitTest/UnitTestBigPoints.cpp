...
namespace UnitTestBigPoints
{
   static constexpr double epsilon{ 1e-2 };
   static constexpr double multiplier{ 1e+12 };
   ...
   public:
      ...
      TEST_METHOD(EquilateralTriangleAxes)
      {
         std::vector<Axis> axes
         {
            { multiplier * Point2(0.5,0), Point2(multiplier * 0.5,sqrt(pow(multiplier,2) * 3) / 2.0) },
            { multiplier * Point2(0,0),  Point2(multiplier * 0.75,sqrt(pow(multiplier,2) * 3) / 4.0) },
            { multiplier * Point2(1,0),  Point2(multiplier * 0.25,sqrt(pow(multiplier,2) * 3) / 4.0) },
         };
         Polygon poly
         {
            std::vector<Point2>
            {
               { multiplier * 0, multiplier * 0 },
               { multiplier * 1, multiplier * 0 },
               { multiplier * 0.5, sqrt(pow(multiplier,2)* 3) / 2.0 },
            }
         };
         Assert::IsTrue(compareAxes(axes, poly.findAxesOfSymmetry(epsilon)));
      }
